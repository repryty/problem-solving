...
def dfs(from_vertex: int, graph: Graph, counter: int)->int:
...
for _ in range(M):
    from_id, to_id = [int(i)-1 for i in input().split()]
    graph.add_edge_indirectional(from_id, to_id)

print(dfs(0, graph, 0)-1)